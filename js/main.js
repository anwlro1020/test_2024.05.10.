...
$('.next').on('click', function (event) {
	...
	if (enable_event) {
		// 이벤트가 방안으로 들어옵니다. 하나의 이벤트가 들어오면 거짓이라 문 닫음.
		enable_event = false;
...
// 네비 버튼을 눌렀을 때 발생 조건에 따라서 함수를 호출합니다.
$('.navi>li').on('click', function (event) {
	event.preventDefault();
	// 현재 활성화가 있는 인덱스를 구합니다. => 5
	if (enable_event) {
		enable_event = false;
		let current_index = $('.panel>li').filter('.on').index();
		// ex) 현재 5번이 보이고 저는 2번을 누르는 상태 => 2
		// 타겟인덱스를 구합니다(네비버튼을 누른 가고자하는 타겟) => 2
		let target_index = $(this).index();

		// 중요: 전역변수와 싱크를 맞춰야합니다.
		active_index = target_index;
		// -> 엑티브 인덱스에 타겟 인덱스를 넣는다. 변수를 전역변수에 쏙 집어넣음.
		// -> 디스? 네비의 li 전체 중에 내가 클릭한 곳에 이벤트 동작을 하는 것
